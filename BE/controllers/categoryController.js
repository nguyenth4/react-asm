...
class CategoryController {
    ...
    static async create(req, res) {
        try {
            ...
            if (error.name === 'SequelizeUniqueConstraintError') {
                return res.status(400).json({ error: "Đường dẫn (Slug) đã tồn tại. Vui lòng sử dụng tên hoặc đường dẫn khác!" });
            }
            const message = error.errors ? error.errors.map(e => e.message).join(', ') : error.message;
            res.status(500).json({ error: message });
        }
    ...
    static async update(req, res) {
        try {
            ...
            if (error.name === 'SequelizeUniqueConstraintError') {
                return res.status(400).json({ error: "Đường dẫn (Slug) đã tồn tại. Vui lòng sử dụng đường dẫn khác!" });
            }
            const message = error.errors ? error.errors.map(e => e.message).join(', ') : error.message;
            res.status(500).json({ error: message });
        }
