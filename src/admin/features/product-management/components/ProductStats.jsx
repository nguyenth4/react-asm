...
const ProductStats = ({ products, boxFilter, setBoxFilter }) => {
  ...
  const handleBoxClick = (f) => {
    // Nếu bấm lại ô đang chọn thì hủy (trở về all), nếu không thì chọn ô mới
    if (boxFilter === f) {
      setBoxFilter('all');
    } else {
      setBoxFilter(f);
  ...
  return (
    <div className="mini-stats">
      ...
      <div className="ms" onClick={() => handleBoxClick('lowStock')} style={getStyle('lowStock')}>
      ...
      <div className="ms" onClick={() => handleBoxClick('outOfStock')} style={getStyle('outOfStock')}>
